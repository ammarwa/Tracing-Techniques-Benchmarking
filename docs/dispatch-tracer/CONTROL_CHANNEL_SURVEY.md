...
The `libroc-shim` alternative (see [SHIM_MEMFD_SOCK_DESIGN.md](SHIM_MEMFD_SOCK_DESIGN.md)) takes a fundamentally different approach: the shim is a pure IPC transport layer loaded unconditionally by register. It does zero profiling — the SDK handles all dispatch-table wrapping, correlation IDs, and record generation. The shim proxies the SDK's tool API over a socket, creating internal ring buffers that the SDK writes to and delivering records to the external consumer. This eliminates all three preconditions above: (1) coexistence is managed by the SDK's existing `force_configure` semantics, (2) OMPT is handled by the SDK, not the shim, and (3) no `LD_PRELOAD` needed — register loads the shim unconditionally. The cost is zero hot-path overhead when dormant (just a sleeping thread); the SDK's own overhead applies only after a consumer attaches.
