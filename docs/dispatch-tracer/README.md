...
- **OMPT integration** needs a different touch because OpenMP only scans for `ompt_start_tool` once, at its init. The stub therefore *will* export `ompt_start_tool` (planned — not in the current mock), but its implementation registers no callbacks at startup and only installs real ones at attach time. See [CONTROL_CHANNEL_SURVEY.md § OpenMP / OMPT](CONTROL_CHANNEL_SURVEY.md#openmp--ompt--a-different-registration-path) for the full mechanism.
...
| [SHIM_MEMFD_SOCK_DESIGN.md](SHIM_MEMFD_SOCK_DESIGN.md) | `libroc-shim.so` — IPC transport layer for OOP profiling. Proxies the rocprofiler-sdk tool API over memfd+sock, replaces ptrace attach. SDK does all profiling; shim handles IPC + ring buffers + record delivery. |
...
- **OpenMP OMPT integration** — OMPT tool discovery and callback registration for the late-load stub approach
