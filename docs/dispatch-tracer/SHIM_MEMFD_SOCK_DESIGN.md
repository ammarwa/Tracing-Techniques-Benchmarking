...
- **Ring size as a consumer option**: consumer may want larger rings for heavy workloads. Today the size is fixed at target startup. A `rocp_shim_resize_ring` query could reallocate if both sides agree, but `F_SEAL_GROW` prevents grow-in-place; would need a secondary memfd + handoff. Not a v1 feature. **Mitigation for v1**: the consumer can control the watermark threshold (how full the ring gets before the target kicks an eventfd wake). A lower watermark means the consumer drains more frequently, reducing the effective pressure on a fixed-size ring. Between the per-op filter chain (§13.5, which keeps uninteresting events out of the ring entirely) and a tunable watermark, the fixed ring size should be adequate for most workloads.
