 ...
 *
 * Usage:
 *   rocp_ctrl_mmap --pid <pid> configure   [flags]
 *   rocp_ctrl_mmap --pid <pid> activate
 *   rocp_ctrl_mmap --pid <pid> deactivate
 *   rocp_ctrl_mmap --pid <pid> reconfigure [flags]
 *   rocp_ctrl_mmap --pid <pid> status
 */
#define _GNU_SOURCE
#include <errno.h>
#include <fcntl.h>
...
static void send_cmd(rocp_ctrl_t* ctrl, uint32_t cmd)
{
...
int main(int argc, char** argv)
{
    ...
    if (strcmp(action, "configure") == 0) {
        ...
        send_cmd(ctrl, CMD_CONFIGURE);
        printf("Configured PID %d\n", (int)target);
    } else if (strcmp(action, "activate") == 0) {
        send_cmd(ctrl, CMD_ACTIVATE);
        printf("Activated PID %d\n", (int)target);
    } else if (strcmp(action, "deactivate") == 0) {
        send_cmd(ctrl, CMD_DEACTIVATE);
        printf("Deactivated PID %d\n", (int)target);
    } else if (strcmp(action, "reconfigure") == 0) {
        write_config(ctrl, argc, argv);
        send_cmd(ctrl, CMD_RECONFIGURE);
        printf("Reconfigured PID %d\n", (int)target);
