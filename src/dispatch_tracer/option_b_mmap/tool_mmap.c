/*
 * tool_mmap.c - libmock_sdk_tool_mmap.so
 *
 * The SDK tool library — dlopen'd by librocp_stub_mmap.so at first
 * attach. Exports rocprofiler_configure so the mock SDK's
 * force_configure can discover and invoke us.
 *
 * Links against libmock_sdk.so + libmock_register.so: calling
 * rocprofiler_force_configure in the stub resolves against the mock
 * SDK that is pulled into the address space transitively (the stub
 * dlopens us; we in turn are linked against mock_sdk).
 */
#define _GNU_SOURCE
#include <dlfcn.h>
#include <pthread.h>
...
/* Declared by the stub — we read them to filter events. */
...
/* Open (or reopen) the text output file. Caller holds s_output_lock. */
static void open_output_locked(const char* path)
{
    ...
    if (s_output_file) {
        ...
        fprintf(stderr,
                "[rocp_tool_mmap] could not open output file '%s'\n", path);
    }
}

/* Callback invoked by mock_sdk's wrapper functors. */
static void my_callback(rocprofiler_callback_tracing_record_t record,
                        void* user_data)
{
    rocp_stub_state_t* state = (rocp_stub_state_t*)user_data;

    /* Runtime filter: drop if domain not enabled. */
    ...
    /* Only emit on the enter phase for brevity. */
...
static int tool_initialize(void* fini, void* tool_data)
{
    ...
    if (!state || !state->pending_config) {
        fprintf(stderr, "[rocp_tool_mmap] no stub state available\n");
        return -1;
    }
    const rocp_config_t* cfg = state->pending_config;

    /* Set up output file for text-format trace. */
    pthread_mutex_lock(&s_output_lock);
    ...
    if (st != ROCPROFILER_STATUS_SUCCESS) {
        fprintf(stderr, "[rocp_tool_mmap] create_context failed: %d\n",
                (int)st);
        return -1;
    }

    /* Default: if nothing explicitly enabled, enable HIP so we have a
     * useful behaviour for the smoke test. */
...
static void tool_finalize(void* tool_data)
{
    (void)tool_data;
    int expected = 0;
    if (!atomic_compare_exchange_strong_explicit(&s_finalized,
                                     &expected, 1,
...
/* Also register for atexit in case force_configure doesn't arrange it. */
